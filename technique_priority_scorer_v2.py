...
Removed the fourth criterion (C4: Asset Impact). Uses:
...
class TechniquePriorityScorer:
    ...
    def compute_criteria(self):
        """
        Compute the three prioritization criteria for each technique
        ...
        # Create decision matrix (C1, C2, C3)
        self.decision_matrix = self.df[['C1_Impact', 'C2_Threat_Score', 'C3_Control_Gap']].values

        logger.info(f"\nDecision matrix shape: {self.decision_matrix.shape}")

        # Display statistics for each criterion
        logger.info("\n=== Criterion Statistics ===")
        for col in ['C1_Impact', 'C2_Threat_Score', 'C3_Control_Gap']:
            logger.info(f"\n{col}:")
    ...
    def compute_entropy_weights(self):
        ...
        # Display results
        criteria_names = ['C1 (Impact)', 'C2 (Threat Score)', 'C3 (Control Gap)']
    ...
    def export_results(self, output_file: str = "technique_priority_scores.xlsx"):
        ...
        try:
            with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
                # Sheet 1: Complete results with all columns (without C4)
                cols_to_export = [
                    'Priority_Rank',
                    'Technique ID',
                    'Technique Name',
                ...
                # Keep only columns that exist in the df (defensive)
...
def main():
    ...
    OUTPUT_FILE = "output/technique_priority_scores_v2.xlsx"
...
if __name__ == "__main__":
    ...
    2. Ensure you have the technique statistics file with sheet "Technique Statistics"

    3. Required columns in the sheet:
      * Technique Name
      * Technique ID
      * Number of Software Using Technique
      * Number of Campaigns Using Technique
    ...
    Methodology:
    1. Compute three criteria (C1-C3) for each technique
